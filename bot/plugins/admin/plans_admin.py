...
async def plans_menu_handler(client: Client, cb: CallbackQuery):
    ...
    await cb.edit_message_text(
        "💳 **Manage Plans**\n\nSelect a plan to edit its title, description, or price.",
        reply_markup=admin_plan_select_kb("admin:planedit"),
    )
...
async def plan_edit_menu_handler(client: Client, cb: CallbackQuery):
    ...
    # BUG-6 FIX: guard against plan document missing from DB.
...
async def plan_field_prompt(client: Client, cb: CallbackQuery):
    ...
    await client.send_message(
        cb.message.chat.id,
        f"✏️ Send the new **{field}** for ₹{plan_id} plan now.\n\nSend /cancel to abort.",
    )
...
async def plan_field_capture(client: Client, message: Message):
    ...
    if field == "price":
        if not value.isdigit():
            await message.reply_text("❌ Price must be a whole number. Try again from the admin panel.")
            return
        value = int(value)
